...
int	main(int argc, const char *argv[])
{
    ...
    memset(&p->sfinfo[0], 0, sizeof(p->sfinfo[0]));
	...
	while (!done) {
		...
		switch (ch) {
            ...
            case 'L':
                ...
                sig_filter->init(LPF, NUM_CHN, FRAMES_PER_BUFFER);
                p->pe = sig_filter;
                break;
            case 'H':
                str = (char *)"High Pass Filter";
                sig_filter->init(HPF, NUM_CHN, FRAMES_PER_BUFFER);
                p->pe = sig_filter;
                break;
            case 'D':
                str = (char *)"DRC";
                sig_drc->init(LPF, NUM_CHN, FRAMES_PER_BUFFER);
                p->pe = sig_drc;
                break;
            case 'Q':
                /* quit */
    ...
    void *userData)
{
    ...
    if (p->sndfile[0] != NULL) {
        unsigned int count = sf_readf_float (p->sndfile[0], input, num_frames);
        if (count != num_frames) {
            /* rewind to beginning */
